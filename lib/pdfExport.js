export async function generateMissionReportPdf({ identity, missionReportText, tracker }) {
  ...
  doc.setFontSize(16);
  doc.text("Mission Report", margin, y);
  y += 26;

  doc.setFont("helvetica", "normal");
  doc.setFontSize(11);
  const bodyLines = doc.splitTextToSize(missionReportText, pageWidth - margin * 2);
  ...
  for (const day of photoDays) {
    ...
    doc.setFont("helvetica", "bold");
    doc.setFontSize(12);
    doc.text(`Battle Evidence - Hari ke-${day.day}`, margin, y);
    y += 16;
  ...
  doc.save(`mission-report-${fileSafeNim}.pdf`);
}
